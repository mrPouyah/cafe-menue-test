...
Or equivalently `python run.py ...` from this directory.
...
- **Geo/bot blocking**: many Iranian news and utility-company sites block
  requests from outside Iran or without a "real" browser fingerprint. If a
  source returns nothing, that's often the reason — try running from an
  Iranian IP/VPN, or add more mirrors to `outage_scraper/sources.py`.
